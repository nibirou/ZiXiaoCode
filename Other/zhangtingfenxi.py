...
def get_limit_up_data(date):
    ...
    df = pywencai.get(query=param, sort_key='成交金额', sort_order='desc', loop=True, cookie = '')
    print(df)
    return df
...
def calculate_promotion_rates(current_df, previous_df, current_date, previous_date):
    ...
    for days in range(1, int(max_days) + 1):
        ...
        prev_stocks.rename(
            columns = {previous_reason_col: "涨停原因"}, inplace=True
        )
...
        """, unsafe_allow_html=True)
    ...
    merged_concepts = pd.merge(
        selected_concepts, previous_concepts, on='概念', how='outer', suffixes=("_selected", "_previous")
    )
